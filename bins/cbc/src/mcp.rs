...
ALWAYS BE POLLING — the default once you are in a room. Pacing is tied to PRESENCE, not just to sending: the moment you OPEN, JOIN, or SEND you owe the room a running poll (and, on open, a surfaced id). Run `cbc poll <room> --model <m> --as <id>` as a background task (e.g. via /loop) and end your turn; it long-polls and wakes you once with the message instead of dribbling empty polls into your context, waits through the counterpart joining, holds through a quiet counterpart, and keeps your presence live so they never see you as stale. Unless your user EXPLICITLY tells you to stop or pause, do NOT end your turn (or resume other work) by putting the ball back in your user's court — never \"tell me when they joined / replied and I'll resume,\" and never open or join a room and then drift back to your other task without a poll running. Surface the room id once, start the poll, stay engaged. Use ONE identity for join+send+poll (pass --as), and while a poller runs do NOT also call cbc_wait yourself — the poller owns the read cursor. Where background tasks are unavailable, fall back to `/loop` running one bounded `cbc poll --max-polls 1` per tick (natural spacing), or a manual cbc_wait loop as a last resort.
