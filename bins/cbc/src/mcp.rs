...
impl CbcMcp {
    ...
    #[tool(
        description = "Post a msg to a room. Repo and cwd are auto-detected; you supply the model. Identity is one stable participant per session — omit `as` to inherit it (the default); pass the HANDLE you were given to resume after a lost session. Omit `to` to broadcast to all participants. You must have joined first. Write a SUBSTANTIVE turn (conclusion + what you verified and how + the ask) — terse turns cause stale, talking-past-each-other loops. Returns {seq} as JSON"
    )]
    async fn cbc_send(
        &self,
        Parameters(SendArgs {
    ...
    #[tool(
        description = "Emit a sentinel (out-of-band signal) to a room. `type` is waiting_user (you are consulting your user) or fold. waiting_user requires `severity` (low|med|high) and `question_text` (the question you are asking your user); fold takes neither. Repo and cwd are auto-detected; you supply the model. Identity is one stable participant per session — omit `as` to inherit it (the default); pass the HANDLE you were given to resume after a lost session. You must have joined first. Sentinels do not count toward the caps. Returns {seq} as JSON"
    )]
    async fn cbc_signal(
        &self,
    ...
    #[tool(
        description = "Pause a room (park it; only an explicit wake resumes it). Optional `reason` is recorded in the audit log. Repo and cwd are auto-detected; you supply the model. Identity is one stable participant per session — omit `as` to inherit it (the default); pass the HANDLE you were given to resume after a lost session. You must have joined first. Non-idempotent: pausing an already-paused room is a 409. Returns {state} as JSON"
    )]
    async fn cbc_pause(
        &self,
    ...
    #[tool(
        description = "Wake a paused (or idle) room back to active. Repo and cwd are auto-detected; you supply the model. Identity is one stable participant per session — omit `as` to inherit it (the default); pass the HANDLE you were given to resume after a lost session. You must have joined first. Non-idempotent: waking an already-active room is a 409. Returns {state} as JSON"
    )]
    async fn cbc_wake(
        &self,
        Parameters(WakeArgs {
